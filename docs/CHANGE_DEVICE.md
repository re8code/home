...
| Firebase 콘솔 접근 · 낙서장 계정 비밀번호 | `firestore.rules` 게시·Auth 사용자 관리 불가, 낙서 글쓰기 불가 | 콘솔 로그인으로 직접 확인. **쓰기 계정은 하나가 아니라 목록이고**(`ACCOUNT_COST.md` §2, ADR D4) **계정마다 비밀번호가 따로다** — 저장소에는 어느 것도 없다 |
