...
- 최종 갱신: 2026-09-05 (또 다른 장비로 옮겨와 §7 함정 11 추가 — Owner 계정이 목록에 있어도 콘솔 대조가 건너뛰어지던 것을 스크립트가 ⚠️로 드러내도록 수정). 이전 갱신: 2026-08-29 (다른 장비로 옮겨와 이 프로토콜을 처음 실제 적용하며 §3 도입부·§6 함정 2건 보강 — 상세: `../report/2026-08-29-device-move-check.md`. 최초 작성 근거는 `../report/2026-08-29-repo-path-move-cleanup.md`)
...
- [ ] **테스트용 로그인 정리** — `gcloud`·`firebase`·`docker`·`supabase` 등에 남은 계정을 전부 내린다(`CLAUDE.md` "여러 장비 간 작업 연속성"). `gcloud auth revoke <계정>` · `firebase logout <계정>`.
      **떠날 때 전부 내리고, 새 장비에서 §5 준비물로 다시 로그인하는 순환**이다 — 특히 gcloud Owner 계정을 쓸 수 없으면 점검의 콘솔 대조 두 항목(게시된 규칙·Auth 사용자)이 **실패가 아니라 건너뛰어져** 확인이 안 된 상태로 남는다. 2026-09-05부터 스크립트가 이 건너뜀을 ⚠️로 드러내고 어느 계정이 왜 못 쓰였는지까지 찍는다(§7 함정 11).
...
# pgrep 은 샌드박스/격리 환경에서 아무 프로세스도 못 찾는 경우가 있다(§7 함정 1).
# ps 기반 검사를 1순위로 두고 pgrep 은 보조로만 쓴다.
RUNNING=0
if ps -eo comm= 2>/dev/null | grep -qx 'claude'; then RUNNING=1; fi
if ps -eo args= 2>/dev/null | grep -q '[b]in/claude'; then RUNNING=1; fi
if pgrep -x claude >/dev/null 2>&1 || pgrep -f "bin/claude" >/dev/null 2>&1; then RUNNING=1; fi
if [ "$RUNNING" = "1" ]; then
  echo "중단: Claude Code 를 모두 종료한 뒤 다시 실행하세요." >&2
  echo "      (실행 중 ~/.claude.json 을 고치면 종료 시 덮어써져 변경이 사라집니다)" >&2
  exit 1
fi
...
**저장소에서 빠지는 것은 `.project-meta.json` 하나뿐이다**(2026-09-03 추가 — 그 전까지는 `.gitignore`가 `.DS_Store` 한 줄뿐이라 빠지는 파일이 아예 없었다). Firebase 설정(`assets/js/firebase-config.js`)조차 추적된다 — 정적 사이트라 브라우저로 어차피 공개되는 값이기 때문이다(웹 `apiKey`는 비밀이 아니며, 접근 통제는 `firestore.rules`가 한다). 그래서 "새 장비에서 파일을 복원해야 하는" 상황은 없고, 대신 **저장소 밖에 있는 것들**을 갖췄는지만 확인하면 된다.
...
| Firebase 콘솔 접근 · 낙서장 계정 비밀번호 | `firestore.rules` 게시·Auth 사용자 관리 불가, 낙서 글쓰기 불가 | 콘솔 로그인으로 직접 확인. 쓰기 계정은 `ACCOUNT_COST.md` §2의 목록이고 **비밀번호는 저장소에 없다** |
| `gcloud` 자격 증명 (Owner 계정) | `check-device.sh`의 **게시된 규칙·Auth 사용자 대조가 건너뛰어진다**(실패는 아니다 — ⚠️로 표시된다) | **`gcloud auth list`에 이름이 있는 것만으로는 부족하다**(§7 함정 11) — 스크립트를 돌려 `콘솔 대조` 줄이 ✅인지 본다. ⚠️면 그 줄이 지목한 계정으로 `gcloud auth login`을 다시 한다(`ACCOUNT_COST.md` §2의 Owner 계정) |
| 닷홈 계정 | 도메인·DNS 변경 불가 | 콘솔 로그인으로 직접 확인 |
| `.project-meta.json` (매니페스트) | 계정·비용 취합 도구가 이 저장소를 건너뛴다 | `ls .project-meta.json`. 없으면 **값을 옮겨오는 게 아니라 `tasks/매니페스트-생성-지시서.md`로 다시 생성한다** — 문서에서 파생되는 사본이라 그게 더 정확하다. **순서 주의(2026-09-12)** — 지시서 0단계가 `gh`·`gcloud` 로그인을 먼저 요구하고 미로그인이면 거기서 멈춘다. 이 표의 위쪽 준비물을 갖춘 뒤에 돌려야 왕복이 한 번 준다 |
...
1. **`pgrep`이 아무 프로세스도 못 찾는 환경이 있다 — 안전장치가 조용히 통과된다.** 2026-08-31 실제로 당했다: Claude Code가 실행 중인데 `pgrep -x claude`·`pgrep -f bin/claude` 둘 다 빈손이라 스크립트가 그대로 실행돼 버렸다(같은 시점 `ps -eo comm=`으로는 `claude` 프로세스가 정상적으로 보였다 — 샌드박스/격리 환경의 `pgrep` 한계로 보인다). **가드는 `ps` 기반을 1순위로 두고 `pgrep`은 보조로만 쓴다**: `ps -eo comm= | grep -qx 'claude'` + `ps -eo args= | grep -q '[b]in/claude'`. §4 템플릿도 같이 고쳐 뒀다.
2. **Claude Code는 `~/.claude.json`을 메모리에 들고 있다가 종료 시 되쓴다.** 실행 중에 편집하면 그 변경이 조용히 사라진다. 스크립트가 `pgrep`으로 중단하는 이유이고, **세션 종료 후 한 번 더 돌려** 되살아난 항목이 없는지 확인해야 하는 이유다(멱등이라 안전).
3. **`history.jsonl`은 실행 중에도 append된다.** `os.replace`로 inode를 갈면 실행 중인 세션의 append가 사라진 inode로 들어간다. **제자리 재작성**(`r+` → `seek(0)` → `write` → `truncate`)으로 처리한다.
4. **`display` 필드는 건드리지 않는다.** 사용자가 타이핑한 프롬프트 본문 안에 옛 경로가 들어 있을 수 있는데, 그건 경로 키가 아니라 발화 기록이다. 바꾸면 과거 발화를 위조하게 된다. 치환 대상은 `"project":"..."` 뿐이다.
5. **`"project":"..."`에는 콜론 뒤 공백이 없다.** 샘플을 `json.dumps`로 다시 찍어보면 공백이 붙어 보여 `"project": "..."`로 착각하기 쉽다 — **원문을 직접 봐야 한다**.
6. **세션 트랜스크립트를 지워도 `file-history/<세션ID>/`는 함께 지워지지 않는다.** 실제로 19MB·561파일이 고아로 남아 있었다.
7. **저장소가 사라진 프로젝트 항목**은 이관이 아니라 삭제 대상이다. 다만 다른 장비에 있을 수 있으니 지우기 전에 확인한다.
8. **`GONE`에 넣으면 `history.jsonl`의 그 프로젝트 프롬프트 이력까지 통째로 지워진다.** 경로 잔재는 남아 있어도 해롭지 않은 반면 입력 이력은 복구할 수 없으니, 기본값은 `GONE=()`로 두고 정말 지울 것만 사용자 확인을 받아 넣는다.
9. **스크립트는 `~/`에 두되, 사라질 수 있다는 전제로 다룬다.** `~/migrate-claude-path.sh`는 저장소 밖이라 git으로 동기화되지 않는다 — 장비를 바꾸거나 홈을 정리하면 없어지고, 세션 스크래치패드(`/private/tmp/...`)에 임시로 둔 사본은 세션이 끝나면 함께 사라진다(2026-08-31 둘 다 사라진 것을 확인). 다시 만들 때는 이 문서 §4 템플릿에서 꺼내 `PAIRS`만 그 장비의 실제 잔재로 채우면 되므로, **정본은 언제나 이 문서**다.
11. **`gcloud auth list`에 Owner 계정이 보여도 콘솔 대조는 건너뛸 수 있다.** 2026-09-05 새 장비에서 실제로 당했다 — 목록에는 Owner(`won@re8code.com`)가 멀쩡히 있었지만 refresh 토큰이 만료돼(`Reauthentication failed. cannot prompt during non-interactive execution`) 토큰 발급이 실패했고, 활성 계정(`cheyeyne@gmail.com`)은 토큰은 나오지만 `graffiti-3b1fc`에 권한이 없어 403이었다. 둘 다 `continue`로 넘어가 함수가 아무것도 출력하지 않고 끝났다 — **화면이 전부 초록인데 두 항목은 확인되지 않은 상태**. 그래서 스크립트가 이제 마지막에 ⚠️ `콘솔 대조`를 찍고 *재인증 필요* / *프로젝트 권한 없음* 계정을 각각 이름으로 지목한다. **판정 기준은 `gcloud auth list`가 아니라 그 줄이 ✅인지**다.
12. **스크립트를 `~/`에 쓰지 못하는 경우가 있다.** 권한 설정에 따라 저장소 밖 파일 생성이 막힐 수 있는데(2026-08-29 실제 발생), 그때는 세션 스크래치패드에 만들어 **사본 검증까지 마친 뒤** 사용자에게 `~/`로 복사해 실행하도록 넘긴다 — 검증 없이 넘기지 않는다.
