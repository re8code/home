...
- [ ] **테스트용 로그인 정리** — `gcloud`·`firebase`·`docker`·`supabase` 등에 남은 계정을 전부 내린다(`CLAUDE.md` "여러 장비 간 작업 연속성"). `gcloud auth revoke <계정>` · `firebase logout <계정>`.
      **떠날 때 전부 내리고, 새 장비에서 §5 준비물로 다시 로그인하는 순환**이다 — 특히 gcloud Owner 계정이 없으면 점검의 콘솔 대조 두 항목(게시된 규칙·Auth 사용자)이 **실패가 아니라 조용히 건너뛰어져** 화면이 전부 초록인데도 확인이 안 된 상태가 된다.
...
| `gcloud` 자격 증명 (Owner 계정) | `check-device.sh`의 **게시된 규칙·Auth 사용자 대조가 조용히 건너뛴다**(실패는 아니다) | `gcloud auth list`로 확인. 없으면 `gcloud auth login`으로 `ACCOUNT_COST.md` §2의 Owner 계정을 추가한다 |
...
10. **스크립트를 `~/`에 쓰지 못하는 경우가 있다.** 권한 설정에 따라 저장소 밖 파일 생성이 막힐 수 있는데(2026-08-29 실제 발생), 그때는 세션 스크래치패드에 만들어 **사본 검증까지 마친 뒤** 사용자에게 `~/`로 복사해 실행하도록 넘긴다 — 검증 없이 넘기지 않는다.
