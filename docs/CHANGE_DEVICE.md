...
8. **스크립트는 `~/`에 두되, 사라질 수 있다는 전제로 다룬다.** `~/migrate-claude-path.sh`는 저장소 밖이라 git으로 동기화되지 않는다 — 장비를 바꾸거나 홈을 정리하면 없어지고, 세션 스크래치패드(`/private/tmp/...`)에 임시로 둔 사본은 세션이 끝나면 함께 사라진다(2026-08-31 둘 다 사라진 것을 확인). 다시 만들 때는 이 문서 §4 템플릿에서 꺼내 `PAIRS`만 그 장비의 실제 잔재로 채우면 되므로, **정본은 언제나 이 문서**다.
9. **스크립트를 `~/`에 쓰지 못하는 경우가 있다.** 권한 설정에 따라 저장소 밖 파일 생성이 막힐 수 있는데(2026-08-29 실제 발생), 그때는 세션 스크래치패드에 만들어 **사본 검증까지 마친 뒤** 사용자에게 `~/`로 복사해 실행하도록 넘긴다 — 검증 없이 넘기지 않는다.
