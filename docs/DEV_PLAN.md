...
### Phase 2.7 — 자료구조 파일럿 (착수, 2026-08-23)
...
- 자료구조 6개 카드(전용 SVG 아이콘 + "준비중" 배지)를 `#ds-swiper`에 추가 완료 — 다만 아직 `<a>` 링크가 아닌 `<div>`(클릭 불가)로, 상세 시각화 페이지는 하나도 만들지 않은 상태. OJ/LMS 등 서브도메인 미오픈 시 쓰던 "준비중" 배지 관례를 카드 단위로 재사용.
- 남은 작업: 6종 중 어느 것부터 만들지 결정(정렬/탐색 파일럿처럼 한 종씩 순차 진행 예정), 상세 페이지 완성 시마다 해당 카드를 실제 링크로 전환.
