...
### 확정적으로 나가는 돈 — 도메인, 그리고 확인이 필요한 것 하나

이 저장소가 쓰는 서비스 중 **정기 결제가 확인된 것은 `recode.ai.kr` 도메인 갱신 하나**다. 나머지는 전부 무료 한도 안에서 돌고 있다.

**다만 `re8code.com` 계정 자체는 확인이 필요하다(2026-09-05, 미확인).** 조직 귀속(ADR D5) 작업 중 `re8code.com`의 MX 레코드가 `ASPMX.L.GOOGLE.com` 등 **Google을 가리키는 것**을 확인했다 — `won@`·`biz@` 메일함이 Google에 있다는 뜻이다. Cloud Identity Free에는 Gmail이 포함되지 않으므로 **Google Workspace(좌석당 유료)일 가능성이 높지만**, 무료로 남아 있는 레거시 에디션도 존재하므로 **단정하지 않는다.** 확인은 관리 콘솔의 구독 화면에서만 가능하다.

이것이 중요한 이유는 금액 자체보다 **의존 구조** 때문이다 — 이 계정이 끊기면 조직(`438985008538`) 관리 권한, Firebase 콘솔 Owner(`won`·`biz`), 낙서장 쓰기 계정이 **한꺼번에** 영향을 받는다. 도메인(닷홈)과 함께 이 생태계에서 잃으면 가장 곤란한 두 축이다.
...
- **1:1 상담 Google Forms는 여전히 `triwon20@gmail.com` 소유다.** Firebase 권한 이전과 전혀 다른 절차이고, 이 계정을 정리할 때 함께 옮기지 않으면 상담 접수가 끊긴다. **방향은 정해졌다(2026-09-10)** — Firestore로 옮기는 안은 스팸·알림·개인정보 대가가 커서 보류하고, **Drive 소유권만 `won@re8code.com`으로 이전**한다(게시 URL은 폼 ID 기반이라 바뀌지 않아 저장소를 손대지 않는다). 절차와 막힐 수 있는 지점은 `report/2026-09-10-forms-ownership-plan.md`.
...
- 저장소 루트의 `.project-meta.json`(기계 판독용 매니페스트, gitignore 대상)은 **이 문서에서 파생된 사본**이다 — 정본은 언제나 이 문서이고, 계정·서비스가 바뀌면 매니페스트를 다시 생성한다(`report/2026-09-03-project-manifest.md`). 금액은 이 문서에 없으므로 매니페스트에서도 비어 있고, 그 칸은 사람이 콘솔 값을 보고 채운다.
