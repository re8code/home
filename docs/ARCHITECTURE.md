...
`recode.ai.kr`은 단일 서비스가 아니라, 메인 사이트(이 저장소) + 서로 독립적으로 개발되는 4개 서브도메인 프로젝트로 구성된 느슨한 연합 구조다. **2026-09-13 재편 결정(D6)**: 메인 사이트는 학습 콘텐츠를 `student.recode.ai.kr`(가칭, 별도 프로젝트)로 내보내고 학생·기업 양쪽을 잇는 **대문 1장**으로 줄어들 예정이다 — 아래 §2~§6은 재편 전(현재 배포본) 기준이며, `student`가 세워져 이관이 실행되는 시점에 갱신한다. 메인 사이트는 빌드 도구 없는 정적 사이트이고, 유일한 동적 기능("원장님의 낙서" 게시판)만 Firebase를 서버리스 백엔드로 사용한다.
...
- **프로젝트**: Firebase 프로젝트 `graffiti-3b1fc` (설정값은 `assets/js/firebase-config.js`). **운영 계정은 `won@re8code.com`·`biz@re8code.com`이다(2026-09-01, ADR D4) — 콘솔 Owner는 둘, 낙서장 쓰기는 `won@re8code.com` 하나.** 콘솔 IAM과 Authentication 사용자는 원래 다른 레이어라, 여기서는 의도적으로 계정 수를 다르게 둔다. 옛 `triwon20@gmail.com`은 이행 중 함께 허용해 두었다가 검증 후 제거했다(`ACCOUNT_COST.md` §2).
...
### D6. `recode.ai.kr`(`home`)은 **학교가 아니라 대문**이다 — 학생 축은 `student` 단일 사이트로 합치고, 조직은 「에이전시」·최상위 등급은 「에이전트」로 부른다

- **맥락**: 형제 저장소 `../business`의 ADR **D30**(2026-09-11)이 세 축을 정했다 — `home`(학교: 입학→교육·훈련) · `lancer.recode.ai.kr`(관문→풀→눈금→독립) · `business`(기업). 이틀 뒤(2026-09-13) 이 저장소에서 재편을 논의하며 두 가지가 드러났다. ① `home`이 "학교"이면 **학생과 기업 양쪽을 잇는 대문** 역할을 할 수 없다 — 학습 콘텐츠 24장이 앞을 막고 있어 기업 방문자에게는 남의 집이다. ② 학교(`home`)와 관문·풀(`lancer`)이 **다른 사이트**에 있으면, D30 자신이 진단한 문제("브릿지 서비스인데 브릿지가 안 보인다")가 학생 입장에서는 그대로 남는다 — 사다리는 한 화면에 있어야 사다리다. 그리고 용어 충돌 하나 — `business/docs/Philosophy.md` §1은 "우리는 에이전트다"(선수-에이전트)라고 쓰는데, 이번에 정한 최상위 등급명도 "에이전트"(일당백 단독 임무 수행자)이고, 사이트 전체에 "AI Agent"가 이미 깔려 있어 한 단어가 세 뜻을 갖게 됐다.
- **결정**:
  - **`home` = 대문.** 학생·기업 양쪽의 pain point와 그 사이에 선 우리의 자리를 말하는 랜딩 **1장**으로 줄인다. 핵심 주장은 보증 공식 — *검증받은 학생 + 회사 내부 베테랑 + AI Agent = 기업이 원하는 품질의 보증* — 이고, 그래서 우리가 **유일한 길목**이라는 것. 무게는 대칭이 아니라 **학생 쪽에 둔다**(사업의 핵심 취지가 "수많은 학생이 AI 시대에 세상으로 나오는 더 안전한 관문"이므로) — 기업에게 거는 말은 거래 제안이 아니라 **이 공동체에 동참하자는 초대**다(`Philosophy.md` §6 첫 항목 "읍소 vs 에이전트"의 답: 읍소도 제안도 아닌 동참). 학습 콘텐츠 24장(`lang-*` 5 · `algo-*` 14 · `web-*` 5)·`about.html`의 4개 레이어·GNB의 Online Judge/Class/VOD 링크는 전부 `student`로 나간다. 낙서장의 소속은 미정(대문에 두면 흡입력을 깎으므로 `student`의 "원장 칼럼" 쪽이 유력).
  - **`student.recode.ai.kr`(가칭) = 학생 축 전부.** D30의 `home`(학교)과 `lancer`(관문→풀→독립)를 **하나로 합친 것**이다. 별도 저장소·별도 프로젝트(회원·등급·계약·정산이 있어 정적 사이트로는 안 된다 — 공개 층은 이 저장소의 정본 `partials/`·`base.css`·`tailwind-config.js` 패턴을 복제해 정적으로 시작하고, 회원 층은 앱). `lancer.recode.ai.kr`이라는 이름은 쓰지 않는다.
  - **`business` = 변화 없음.** D30 그대로 — 기업 전용, 학생 섹션 없음, 학생에게는 "브랜드의 무게"(증거)로 기능.
  - **등급 4단계**: `학생 → 훈련생 → 랜서 → 에이전트`. 학생·훈련생까지는 **수업료**(훈련생이 의대의 인턴에 해당), 랜서부터는 실전 투입이라 **수익배분·별도 계약**. 훈련생→랜서는 **우리의 승인**(관문 — 학생·훈련생 전 과정을 지켜본 관찰 기록이 평가 자산), 랜서→에이전트는 **면허**(007의 라이선스 — 단독 투입 자격). 에이전트 = "진정한 디지털 노마드". 눈금(동행→위임→책임)·강등 없음·이름 공개 = 독립 등 `Philosophy.md` §3의 규칙은 그대로 상속한다.
  - **용어**: 조직 = **에이전시**(Recode Agency — 스포츠의 소속사이자 007의 MI6, "우리는 에이전트다"의 뜻은 그대로 두고 명사형만 정정) · 사람의 최상위 등급 = **에이전트** · AI는 항상 영문 **"AI Agent"**로 표기하고 셋이 한 문장에 겹칠 때만 "AI 스태프"(`Philosophy.md` §0 3번의 "AI-Agent 스텝"이 근거).
  - **순서**: `student`를 먼저 세우고 → 콘텐츠를 옮긴 뒤 → 마지막에 `home`을 대문으로 깎는다. **`home`을 먼저 깎지 않는다** — 그 사이 `recode.ai.kr`이 빈 집이 된다. 그 전까지 이 저장소에서 하는 일은 문서 기록과 `src/mockup.html` 시안뿐이고 `index.html`은 손대지 않는다(`DEV_PLAN.md` Phase 5).
  - **랜딩 카피에 걸리는 제약** — `business/docs/Philosophy.md`에서 그대로 상속한다: §5 ② **모집이 아니라 선언**("오면 실전 준다"고 말하지 않는다 — 모집은 수주 실적에 후행) · §4 **청년을 전면에 세우지 않는다**(기업 섹션에서는 검증·보증이 먼저, 청년은 뒤 — 가격을 말하지 않아도 품질 의심이 발동한다) · §6 **열린 항목은 카피로 옮기지 않는다**(배분 비율 · "품질 100%"의 운영 정의 · 계약 형태(도급 vs 대여 뉘앙스) — "수익배분이 발생한다"까지만) · §3 **익명**(실존 랜서 사례를 쓰더라도 이름·얼굴 없이) · §5 ③ **지어내지 않는다**.
- **트레이드오프**: **D30을 이틀 만에 개정한다.** `business` 저장소의 D30·`Philosophy.md` §6·`tasks/2026-09-11-학생축-논의.md`는 이 저장소에서 고칠 수 없으므로(다른 프로젝트 폴더는 읽기 전용) **그쪽 세션에서 개정 메모 한 항목을 남겨야 한다** — 문구는 `../report/2026-09-13-landing-restructure-discussion.md` §6에 준비해 뒀다. 이 저장소는 결국 24장을 내보내고 1장만 남게 되므로 지금까지 쌓은 페이지 구조 서술(`CLAUDE.md` 대부분)이 `student` 쪽으로 따라가야 하고, 정본 `partials/` 패턴이 두 저장소에 한동안 중복된다. **비용은 없다** — 서브도메인은 닷홈 DNS 레코드 하나이고 `student`가 Auth·Firestore를 써도 Spark 안이다(D30의 비용 절과 같다). **되돌릴 조건**: `student`가 세워지기 전에는 이 저장소에서 바뀌는 코드가 없으므로 그때까지는 무비용으로 되돌릴 수 있다. `student`를 세우지 못한 채 시간이 가면 D30의 차선(`home`에 관문 기준 문서 한 장)이 그대로 유효하다.
...
- **결정**: `ADMIN_EMAIL`(문자열)을 `ADMIN_EMAILS`(배열)로 바꾸고, 규칙도 `==`에서 `in [...]`으로 바꾼다. 로그인 모달에 **이메일 입력칸을 추가**하고(직전 주소는 `localStorage`에 남겨 다시 채운다 — 비밀번호는 저장하지 않는다), 목록에 없는 주소는 Firebase에 요청을 보내지 않고 즉시 거절한다. 정본 세 곳(`firebase-config.js`의 `ADMIN_EMAILS` · `firestore.rules` · `check-device.sh`의 `FIXED_ADMIN_MAILS`)의 **목록 일치**를 점검 스크립트가 순서 무관하게 대조한다.
  **두 레이어의 계정 수를 다르게 둔다 — 콘솔 Owner는 `won`·`biz` 둘, 낙서장 쓰기는 `won` 하나.** 관리 권한은 이중화해 한 계정이 잠겨도 프로젝트를 잃지 않되, 쓰기 계정은 늘리지 않는다: 게시글에 작성자 필드가 없어(`id·date·title·views·content`) 어느 계정으로 쓰든 결과가 같고, 계정을 늘리면 구분되는 것 없이 로그인 지점만 늘기 때문이다. 이행은 2단계로 진행했다 — ① `triwon20@gmail.com`과 `won@re8code.com`을 함께 허용해 아무것도 끊기지 않는 상태를 만들고, ② `won`으로 실제 쓰기를 검증한 뒤 `triwon20@gmail.com`을 제거했다(2026-09-01 완료 — 코드·규칙·Authentication·IAM Owner 네 곳 모두).
