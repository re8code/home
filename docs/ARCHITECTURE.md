...
`CLAUDE.md`의 "제1원칙"에 따라 프로젝트 전체의 기술 스택을 이 문서 상단에 정리한다. 왜 이 스택을 유지하는지(트레이드오프)는 `DEV_PLAN.md` §1, 실제 코드 배치는 아래 §4를 참고. 각 항목의 **계정·비용** 측면만 뽑은 요약은 `account-cost.md`에 따로 둔다.
...
_아직 기록된 항목 없음 — 다음 아키텍처 결정부터 `### D1.`으로 시작한다._
