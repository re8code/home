...
> **정정 (2026-09-04)** — 위에 적은 이유("계정 이메일이 들어가므로")는 이 저장소에서 성립하지 않는다. 같은 이메일이 추적되는 문서에 **이미 15곳** 있다(`won@` 9곳 · `triwon20@` 6곳 — 계정 이력을 문서에 상세히 남기는 방식상 당연한 결과다). 즉 ignore가 그 이메일을 지켜주지는 못한다. **결론(커밋하지 않는다)은 유지되지만 근거가 다르다** — 지키는 대상은 지금 들어 있는 값이 아니라 앞으로 들어갈 값이다. `billing_ref`(결제 수단 참조)·`recovery_ref`(2FA 복구 수단)·`account_ref`·실제 금액·도메인 갱신일은 이 파일의 존재 이유 그 자체이면서(위 "남은 이슈" 1번) 아직 전부 `null`인 칸이고, 채워진 뒤 커밋되면 히스토리에서 지우기 어렵다. `.gitignore`의 주석도 이 이유로 고쳤다.
